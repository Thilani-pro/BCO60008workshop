...
#dataset split and recipe

set.seed(123)

volcano_df_split<-initial_split(volcano_df)

volcano_training<-training(volcano_df_split)
volcano_test<-testing(volcano_df_split)


volcano_training%>%count(volcano_type)
...
#extract data from volcano data set of recipe

juice(volcano_prep)

#if you need to apply recipe to new data (which is not in the recipe settings),you need to use bake()
